...
test('existing gold saves gain an empty rock wallet without losing purchases or receipts', () => {
  ...
  assert.equal(parsed.gold, 20); assert.deepEqual(parsed.ranks, {...old.ranks, 'unlock-crossbow':1, 'unlock-hailey':1, 'unlock-sheldon':1, 'unlock-ron':1}); assert.deepEqual(parsed.settledRuns, old.settledRuns);
